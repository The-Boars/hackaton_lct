...
Сервис для **распознавания именованных сущностей (NER)** в тексте. Реализован на **FastAPI**, использует предобученную модель семейства **DeepPavlov (ner_collection3_bert)**. Эндпоинт `/api/predict` принимает текст и возвращает список найденных сущностей с позициями в исходной строке.
...
> ⚠️ При первом запуске скачивается конфигурация/веса модели DeepPavlov — нужен доступ в интернет. Если доступа нет, скачайте заранее и смонтируйте каталог с моделями.
...
1. Установите Docker (и **NVIDIA Container Toolkit**, если хотите GPU).
2. В корне проекта:
   ```bash
...
| `MODEL_NAME` | Имя модели/конфига (используется в `app/model.py`) | `model`                     |
...
### 5.2 `GET /health`
Проверка живости сервиса. Возвращает `200 OK` с простым JSON, например:
```json
{ "status": "ok" }
```
...
- Ответственный: *Канев Алексей / человек*
